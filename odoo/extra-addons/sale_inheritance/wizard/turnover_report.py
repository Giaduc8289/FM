...
class SaleTurnoverWizard(models.TransientModel):
    _name = 'sale.report.turnover.wizard'
    _rec_name = 'date_start'

    date_start = fields.Date(string='Start Date')
    date_end = fields.Date(string='End Date')
    # nhacungcap = fields.Many2one("res.partner", "Supplier", domain="[('code', 'like', 'NCC%')]")
    ...
    def get_report(self):
        ...
        action = self.env.ref('sale_inheritance.action_report_sale_order_turnover').report_action(self, data=data)
        return action


class ReportSaleTurnover(models.AbstractModel):
    _name = 'report.sale_inheritance.report_sale_turnover_document'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        query = """
                        ...
                        From sale_order dh Left Join sale_order_line dhct on dhct.order_id=dh.id
                         ...
                         where (am.move_type = 'out_invoice')  {where_string}
        ...
        for line in data_puback:
            ...
            _ids.append(self.env['sale.report.turnover.wizard'].create(data_row).id)

        docs = self.env['sale.report.turnover.wizard'].browse(_ids)
